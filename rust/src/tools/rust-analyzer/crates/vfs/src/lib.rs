...
impl Vfs {
    ...
    /// Returns the content hash currently associated with `file_id`.
    ///
    /// This is useful for cheaply rejecting background work that was prepared from an older VFS
    /// snapshot without copying the file contents back onto the event-loop thread.
    pub fn file_content_hash(&self, file_id: FileId) -> Option<u64> {
        match self.get(file_id) {
            FileState::Exists(hash) => Some(hash),
            FileState::Deleted | FileState::Excluded => None,
        }
    }
