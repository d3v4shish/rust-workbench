...
use super::{DiagnosticsMapConfig, Fix, OwnershipWrapperFix};

fn ownership_wrapper_fix(
    code: Option<&crate::flycheck::DiagnosticCode>,
) -> Option<OwnershipWrapperFix> {
    match code.map(|code| code.code.as_str()) {
        Some("borrowck_wrapper_arc") => Some(OwnershipWrapperFix::Arc),
        Some("borrowck_wrapper_arc_mutex") => Some(OwnershipWrapperFix::ArcMutex),
        Some("borrowck_wrapper_arc_rw_lock") => Some(OwnershipWrapperFix::ArcRwLock),
        Some("borrowck_wrapper_mutex") => Some(OwnershipWrapperFix::Mutex),
        Some("borrowck_wrapper_rc") => Some(OwnershipWrapperFix::Rc),
        Some("borrowck_wrapper_ref_cell") => Some(OwnershipWrapperFix::RefCell),
        Some("borrowck_wrapper_rc_ref_cell") => Some(OwnershipWrapperFix::RcRefCell),
        Some("borrowck_wrapper_rw_lock") => Some(OwnershipWrapperFix::RwLock),
        Some("borrowck_wrapper_all") => Some(OwnershipWrapperFix::All),
        _ => None,
    }
}
...
fn primary_location(
    ...
    location(config, workspace_root, select_primary_span(config, workspace_root, span), snap)
}

/// Picks the project-local expansion site used for the primary diagnostic.
///
/// rustc frequently reports a span in `std` or `alloc` first and the user's macro invocation
/// later in the expansion chain. Editors cannot attach a standalone diagnostic to those sysroot
/// files when they are not part of the open worktree, so prefer the project span here.
fn select_primary_span<'a>(
    config: &DiagnosticsMapConfig,
    workspace_root: &AbsPath,
    ...
    for span in span_stack.clone() {
        if is_workspace_span(config, workspace_root, span) {
            return span;
        }
    }

    // Fall back to the outermost macro invocation if no suitable span comes up.
    span_stack.last().unwrap()
}

fn is_workspace_span(
    config: &DiagnosticsMapConfig,
    workspace_root: &AbsPath,
    span: &DiagnosticSpan,
) -> bool {
    !is_dummy_macro_file(&span.file_name)
        && resolve_path(config, workspace_root, &span.file_name).starts_with(workspace_root)
}
...
fn map_rust_child_diagnostic(
    ...
    snap: &GlobalStateSnapshot,
    ownership_wrapper: Option<OwnershipWrapperFix>,
    ...
    if ownership_wrapper.is_none() && !suggested_replacements.is_empty() {
        message.push_str(": ");
        let suggestions =
    ...
    let suggested_fix = if edit_map.is_empty() {
        ...
        Some(Box::new(Fix {
            ranges: spans
                .iter()
                .filter(|&&span| is_workspace_span(config, workspace_root, span))
    ...
    MappedRustChildDiagnostic::SubDiagnostic(SubDiagnostic {
        ...
        suggested_fix,
        materialize: is_workspace_span(config, workspace_root, spans[0]),
    })
}
...
pub(crate) fn map_rust_diagnostic_to_lsp(
    ...
    for secondary_span in secondary_spans {
        let materialize = is_workspace_span(config, workspace_root, &secondary_span);
        let related = diagnostic_related_information(config, workspace_root, secondary_span, snap);
        if let Some(related) = related {
            subdiagnostics.push(SubDiagnostic { related, suggested_fix: None, materialize });
        }
    }

    let ownership_wrapper = ownership_wrapper_fix(diagnostic_code.as_ref());
    let ownership_trigger_ranges: SmallVec<[lsp_types::Range; 2]> = if ownership_wrapper.is_some() {
        primary_spans
            .iter()
            .map(|span| primary_location(config, workspace_root, span, snap).range)
            .collect()
    } else {
        SmallVec::new()
    };
    for child in &children {
        let child =
            map_rust_child_diagnostic(config, workspace_root, child, snap, ownership_wrapper);
        match child {
    ...
    for primary_span in primary_spans {
        let selected_primary_span = select_primary_span(config, workspace_root, &primary_span);
        if !is_workspace_span(config, workspace_root, selected_primary_span) {
            continue;
        }
        let primary_location = location(config, workspace_root, selected_primary_span, snap);
        let message = {
        ...
        for sub in &subdiagnostics {
            ...
            let mut fix = sub.suggested_fix.clone();
            if let Some(fix) = fix.as_mut().filter(|fix| fix.ownership_wrapper.is_some()) {
                fix.ranges.extend(ownership_trigger_ranges.iter().copied());
            }
            diagnostics.push(MappedRustDiagnostic {
...
mod tests {
    ...
    #[test]
    fn ownership_wrapper_multipart_fix() {
        let diagnostic: crate::flycheck::Diagnostic = serde_json::from_str(
            r#"{
                "message": "compiler validated an ownership-wrapper rewrite",
                "code": { "code": "borrowck_wrapper_ref_cell", "explanation": null },
                "level": "help",
                "spans": [{
                    "file_name": "src/main.rs", "byte_start": 20, "byte_end": 26,
                    "line_start": 2, "line_end": 2, "column_start": 9, "column_end": 15,
                    "is_primary": true, "text": [], "label": "ownership wrapper candidate",
                    "suggested_replacement": null, "suggestion_applicability": null,
                    "expansion": null
                }, {
                    "file_name": "src/main.rs", "byte_start": 50, "byte_end": 56,
                    "line_start": 3, "line_end": 3, "column_start": 5, "column_end": 11,
                    "is_primary": true, "text": [], "label": "ownership error occurs here",
                    "suggested_replacement": null, "suggestion_applicability": null,
                    "expansion": null
                }],
                "children": [{
                    "message": "Use RefCell for interior mutability (compiler validated)",
                    "code": null, "level": "help",
                    "spans": [
                        { "file_name": "src/main.rs", "byte_start": 36, "byte_end": 36,
                          "line_start": 2, "line_end": 2, "column_start": 25, "column_end": 25,
                          "is_primary": true, "text": [],
                          "label": null, "suggested_replacement": "::std::cell::RefCell::new(",
                          "suggestion_applicability": "MaybeIncorrect", "expansion": null },
                        { "file_name": "src/main.rs", "byte_start": 42, "byte_end": 42,
                          "line_start": 2, "line_end": 2, "column_start": 31, "column_end": 31,
        ...
        let mapped = map_rust_diagnostic_to_lsp(
            &DiagnosticsMapConfig::default(),
            diagnostic,
            workspace_root,
            &state.snapshot(),
        );
        let fix = mapped.iter().find_map(|diagnostic| diagnostic.fix.as_deref()).unwrap();
        assert_eq!(fix.ownership_wrapper, Some(OwnershipWrapperFix::RefCell));
        assert_eq!(fix.action.is_preferred, Some(false));
        assert_eq!(fix.action.title, "Use RefCell for interior mutability (compiler validated)");
        assert!(fix.ranges.contains(&lsp_types::Range::new(
    ...
    #[test]
    fn sysroot_spans_are_related_but_not_materialized_as_diagnostics() {
        let diagnostic: crate::flycheck::Diagnostic = serde_json::from_str(
            r#"{
                "message": "borrow of moved value: `values`",
                "code": { "code": "E0382", "explanation": null },
                "level": "error",
                "spans": [{
                    "file_name": "src/main.rs", "byte_start": 80, "byte_end": 86,
                    "line_start": 5, "line_end": 5, "column_start": 20, "column_end": 26,
                    "is_primary": true, "text": [], "label": "value borrowed here after move",
                    "suggested_replacement": null, "suggestion_applicability": null,
                    "expansion": null
                }, {
                    "file_name": "library/std/src/macros.rs", "byte_start": 0, "byte_end": 1,
                    "line_start": 1, "line_end": 1, "column_start": 1, "column_end": 2,
                    "is_primary": false, "text": [], "label": "in this macro",
                    "suggested_replacement": null, "suggestion_applicability": null,
                    "expansion": null
                }],
                "children": [{
                    "message": "method is defined here", "code": null, "level": "note",
                    "spans": [{
                        "file_name": "library/alloc/src/vec/mod.rs",
        ...
        let config = DiagnosticsMapConfig {
            remap_prefix: FxHashMap::from_iter([(
                "library/".to_owned(),
                "/rust/toolchain/library/".to_owned(),
            )]),
            ..DiagnosticsMapConfig::default()
        };

        let mapped =
            map_rust_diagnostic_to_lsp(&config, diagnostic, workspace_root, &state.snapshot());

        assert_eq!(mapped.len(), 1, "sysroot spans must not become standalone diagnostics");
        assert_eq!(mapped[0].url.as_str(), "file:///test/src/main.rs");
        let related = mapped[0].diagnostic.related_information.as_ref().unwrap();
        assert_eq!(related.len(), 2, "sysroot context should remain attached to the error");
        assert!(
            related.iter().all(|info| info
                .location
                .uri
                .as_str()
                .starts_with("file:///rust/toolchain/library/"))
        );
    }
