...
pub(crate) fn handle_did_save_text_document(
    ...
    if let Ok(vfs_path) = from_proto::vfs_path(&params.text_document.uri) {
        ...
        if !state.config.check_on_save(Some(sr)) || run_flycheck(state, vfs_path, false) {
            return Ok(());
        }
                ...
                InvocationStrategy::Once => {
                    Box::new(move || {
                        ...
                        if let Some(flycheck) = world.flycheck.first() {
                            flycheck.restart_workspace_with_validation(
                                saved_file,
                ...
                InvocationStrategy::PerWorkspace => {
                    Box::new(move || {
                        ...
                        if let Some((target, root, package)) = target {
                            ...
                            let package_check_allowed = validate_ownership_repairs
                                || target.is_some()
                            ...
                            if package_check_allowed {
                                ...
                                if let Some(idx) = package_workspace_idx {
                                        ...
                                        world.flycheck.iter().find(|fc| fc.id() == idx)
                                    {
                                        ...
                                        flycheck.restart_for_package_with_validation(
                                            package,
                                            target,
                                            workspace_deps,
                                            saved_file.clone(),
...
pub(crate) fn handle_run_flycheck(
    ...
    if let Some(text_document) = params.text_document
        ...
        && run_flycheck(state, vfs_path, false)
    {
        return Ok(());
    }
    // No specific flycheck was triggered, so let's trigger all of them.
