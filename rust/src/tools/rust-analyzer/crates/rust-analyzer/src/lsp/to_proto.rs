...
pub(crate) fn inlay_hint(
    ...
    let rust_workbench_metadata = rust_workbench_inlay_metadata(&inlay_hint);
    ...
    let mut data = match resolve_range_and_hash {
    ...
    let data_object = data.get_or_insert_with(|| Value::Object(Default::default()));
    if let Value::Object(data_object) = data_object {
        data_object.insert("rustWorkbench".to_owned(), rust_workbench_metadata);
    }
...
fn rust_workbench_inlay_metadata(inlay_hint: &InlayHint) -> Value {
    let category = match inlay_hint.kind {
        InlayKind::Adjustment | InlayKind::BindingMode | InlayKind::ClosureCapture => "adjustment",
        InlayKind::Lifetime => "lifetime",
        InlayKind::Ownership => "ownership",
        InlayKind::Drop => "drop",
        InlayKind::Parameter | InlayKind::GenericParameter => "parameter",
        InlayKind::Type | InlayKind::Chaining => "type",
        _ => "other",
    };
    let event_kinds = if inlay_hint.kind == InlayKind::Ownership {
        let label = inlay_hint.label.parts.iter().map(|part| part.text.as_str()).join("");
        let mut kinds = Vec::new();
        for (marker, kind) in [
            ("partial move", "partial_move"),
            ("move", "move"),
            ("&mut", "borrow_mutable"),
            ("&?", "borrow_shared"),
            ("reinit", "reinitialize"),
            ...
            if label.contains(marker) && !kinds.contains(&kind) {
                kinds.push(kind);
            }
        }
        kinds
    ...
    json!({
        "version": 1,
        "category": category,
        "precision": (inlay_hint.kind == InlayKind::Ownership).then_some("analyzer_estimate"),
        "eventKinds": event_kinds,
    })
}

#[cfg(test)]
mod rust_workbench_inlay_tests {
    use super::*;

    fn hint(kind: InlayKind, label: &'static str) -> InlayHint {
        InlayHint {
            range: TextRange::empty(TextSize::new(0)),
            position: ide::InlayHintPosition::After,
            pad_left: false,
            pad_right: false,
            kind,
    ...
    #[test]
    fn metadata_classifies_ownership_lifetime_and_drop_hints() {
        let ownership = rust_workbench_inlay_metadata(&hint(InlayKind::Ownership, "move?"));
        assert_eq!(ownership["category"], "ownership");
        assert_eq!(ownership["precision"], "analyzer_estimate");
        assert_eq!(ownership["eventKinds"][0], "move");

        let lifetime = rust_workbench_inlay_metadata(&hint(InlayKind::Lifetime, "'a"));
        assert_eq!(lifetime["category"], "lifetime");

        let drop = rust_workbench_inlay_metadata(&hint(InlayKind::Drop, "drop(value)"));
        assert_eq!(drop["category"], "drop");
    }
}
