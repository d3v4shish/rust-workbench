...
impl fmt::Display for SupportedModifiers {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        let string = match self {
            ...
            SupportedModifiers::Dropped => "dropped",
            SupportedModifiers::Associated => "associated",
            SupportedModifiers::AttributeModifier => "attribute",
            SupportedModifiers::BorrowEnd => "borrowEnd",
            SupportedModifiers::Borrowed => "borrowed",
            SupportedModifiers::BorrowedMut => "borrowedMut",
...
pub(crate) fn modifier_bit(name: &str) -> Option<u32> {
    SupportedModifiers::iter()
        .position(|modifier| modifier.to_string() == name)
...
mod tests {
    ...
    #[test]
    fn modifiers_fit_the_lsp_bitset() {
        assert!(SupportedModifiers::iter().count() <= u32::BITS as usize);
        assert!(modifier_bit("moved").is_some());
        assert!(modifier_bit("lastUse").is_some());
        assert!(modifier_bit("invalidUse").is_some());
    }
