...
pub(crate) struct GlobalState {
    ...
    /// Monotonic versions for compiler ownership artifacts. Preparation happens on worker
    /// threads and may finish out of order, so commits use these versions to discard stale work.
    pub(crate) ownership_artifact_sequence: u64,
    pub(crate) ownership_file_sequences: FxHashMap<FileId, u64>,
    pub(crate) pending_ownership_artifact_paths: FxHashMap<usize, Vec<std::path::PathBuf>>,
    /// Main-loop-owned mirror of the current VFS content hashes. Ownership artifact commits use
    /// this instead of waiting for the shared VFS lock on the latency-sensitive event loop.
    pub(crate) ownership_file_hashes: FxHashMap<FileId, u64>,
...
pub(crate) struct GlobalStateSnapshot {
    ...
    pub(crate) ownership_diagnostics: crate::diagnostics::OwnershipDiagnostics,
    pub(crate) ownership_events: crate::diagnostics::OwnershipEvents,
    pub(crate) ownership_tutorial_models: crate::diagnostics::OwnershipTutorialModels,
    mem_docs: MemDocs,
    pub(crate) semantic_tokens_cache: Arc<Mutex<FxHashMap<Uri, SemanticTokens>>>,
...
impl GlobalState {
    pub(crate) fn new(sender: Sender<lsp_server::Message>, config: Config) -> GlobalState {
        ...
        let mut this = GlobalState {
            ...
            ownership_artifact_sequence: 0,
            ownership_file_sequences: FxHashMap::default(),
            pending_ownership_artifact_paths: FxHashMap::default(),
            ownership_file_hashes: FxHashMap::default(),
        ...
        let (change, modified_rust_files, workspace_structure_change) =
            self.cancellation_pool.scoped(|s| {
                ...
                for file in changed_files.into_values() {
                    if let Some(hash) = vfs.file_content_hash(file.file_id) {
                        self.ownership_file_hashes.insert(file.file_id, hash);
                    } else {
                        self.ownership_file_hashes.remove(&file.file_id);
                    }
                    // Compiler-derived ownership spans belong to the exact source revision that
                    // produced them. Discard them before applying any edit; rust-analyzer's
                    // estimated ownership hints cover the file until the next flycheck finishes.
                    self.diagnostics.clear_ownership_for(file.file_id);
    ...
    pub(crate) fn snapshot(&self) -> GlobalStateSnapshot {
        GlobalStateSnapshot {
            ...
            ownership_diagnostics: Arc::clone(&self.diagnostics.ownership_diagnostics),
            ownership_events: Arc::clone(&self.diagnostics.ownership_events),
            ownership_tutorial_models: Arc::clone(&self.diagnostics.ownership_tutorial_models),
...
impl GlobalStateSnapshot {
    ...
    pub(crate) fn file_content_hash(&self, file_id: FileId) -> Option<u64> {
        self.vfs_read().file_content_hash(file_id)
    }
