...
config_data! {
    ...
    local: struct LocalDefaultConfigData <- LocalConfigInput ->  {
        ...
        /// Offer experimental ownership-wrapper assists and request compiler-validated `Rc`,
        /// `RefCell`, `Arc`, `Mutex`, and `RwLock` variants from supported Rust toolchains.
        assist_ownershipWrapperSuggestions_enable: bool = false,
        ...
        /// Show ownership-flow hints and coloring. Supported toolchains provide exact MIR events
...
impl Config {
    ...
    pub fn assist(&self, source_root: Option<SourceRootId>) -> AssistConfig {
        AssistConfig {
            ...
            ownership_wrapper_suggestions: *self
                .assist_ownershipWrapperSuggestions_enable(source_root)
    ...
    pub fn inlay_hints<'a>(&self, minicore: MiniCore<'a>) -> InlayHintsConfig<'a> {
        ...
        let ownership_hints = *self.ownership_enable(None);
        ...
        InlayHintsConfig {
            ...
            lifetime_elision_hints: match self.inlayHints_lifetimeElisionHints_enable() {
                ...
                LifetimeElisionDef::Never if ownership_hints => {
                    ide::LifetimeElisionHints::SkipTrivial
                }
            ...
            adjustment_hints: match self.inlayHints_expressionAdjustmentHints_enable() {
                ...
                AdjustmentHintsDef::Never => match self.inlayHints_reborrowHints_enable() {
                    ...
                    ReborrowHintsDef::Never if ownership_hints => ide::AdjustmentHints::BorrowsOnly,
    ...
    pub(crate) fn flycheck(&self, source_root: Option<SourceRootId>) -> FlycheckConfig {
        match &self.check_overrideCommand(source_root) {
            ...
            Some(_) | None => FlycheckConfig::Automatic {
                ...
                ownership_visualization: *self.ownership_enable(source_root),
                ownership_wrapper_suggestions: *self
                    .assist_ownershipWrapperSuggestions_enable(source_root)
                    || *self.ownership_enable(source_root),
            },
...
mod tests {
    ...
    #[test]
    fn ownership_wrapper_suggestions_configure_assists_and_flycheck() {
        let mut config =
            Config::new(AbsPathBuf::assert(project_root()), Default::default(), vec![], None);
        let mut change = ConfigChange::default();
        change.change_client_config(serde_json::json!({
        ...
        (config, _, _) = config.apply_change(change);
        assert!(config.assist(None).ownership_wrapper_suggestions);
        assert!(matches!(
            config.flycheck(None),
            FlycheckConfig::Automatic { ownership_wrapper_suggestions: true, .. }
        ));
    }

    #[test]
    fn ownership_visualization_enables_flow_and_repairs() {
        let mut config =
            Config::new(AbsPathBuf::assert(project_root()), Default::default(), vec![], None);
        let mut change = ConfigChange::default();
        change.change_client_config(serde_json::json!({ "ownership": { "enable": true } }));

        (config, _, _) = config.apply_change(change);
        assert!(config.ownership_enabled(None));
        assert!(config.assist(None).ownership_wrapper_suggestions);
        assert!(matches!(
            config.flycheck(None),
            FlycheckConfig::Automatic {
                ownership_visualization: true,
                ownership_wrapper_suggestions: true,
                ..
            }
        ));
