...
impl Analysis {
    ...
    /// Explains the ownership contract and observable effects of calls connected to `position`.
    pub fn ownership_call_insights(
        &self,
        position: FilePosition,
    ) -> Cancellable<Vec<OwnershipCallInsight>> {
        self.with_db(|db| ownership_insight::ownership_call_insights(db, position))
    }

    /// Explains calls connected to several compiler events in one database transaction and parse.
    pub fn ownership_call_insights_for_positions(
        &self,
