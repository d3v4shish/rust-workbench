...
fn analysis(tcx: TyCtxt<'_>, (): ()) {
    ...
    if sess.opts.unstable_opts.borrowck_autofix.is_some()
        || sess.opts.unstable_opts.borrowck_ownership_events
        || sess.opts.unstable_opts.borrowck_ownership_model.is_some()
        || sess.opts.unstable_opts.borrowck_wrapper_suggestions
    {
        if sess.opts.unstable_opts.borrowck_autofix.is_some()
            || sess.opts.unstable_opts.borrowck_wrapper_suggestions
        {
            rustc_borrowck::build_wrapper_variants(tcx);
        }
        let _ = sess.write_borrowck_repair_plan(
