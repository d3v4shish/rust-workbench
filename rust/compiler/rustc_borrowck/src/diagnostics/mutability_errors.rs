...
use rustc_session::borrowck_repair::{
    BorrowckRepairKind, BorrowckWrapperIntent, BorrowckWrapperRequirement, BorrowckWrapperSource,
};
...
impl<'tcx> MirBorrowckCtxt<'_, '_, 'tcx> {
    pub(crate) fn report_mutability_error(
        ...
        let mut recorded_mutation_binding = false;
        if self.record_interior_mutation_intent(the_place_err.local, span).is_some() {
            recorded_mutation_binding = true;
        }
        if access_place.local != the_place_err.local {
            recorded_mutation_binding |=
                self.record_interior_mutation_intent(access_place.local, span).is_some();
        }
        if !recorded_mutation_binding
            && let Some(body) = self.infcx.tcx.hir_maybe_body_owned_by(self.mir_def_id())
        {
            self.record_interior_mutation_intent_by_span(body.value, span);
        }
        debug!(
                ...
                match self.local_name(local) {
                    Some(name) if !local_decl.from_compiler_desugaring() => {
                        ...
                        if local_decl.ty.is_ref() {
                            err.help(
                                "if mutation through shared references is required by the design, \
                                 consider `Cell` or `RefCell` in single-threaded code, or `Mutex` \
                                 or `RwLock` when the value is shared across threads",
                            );
                        }
                    }
                    _ => {
    ...
    fn construct_mut_suggestion_for_local_binding_patterns(
        ...
        if pat_span == local_decl.source_info.span {
            self.infcx.tcx.sess.record_borrowck_repair_for_binding(
                BorrowckRepairKind::MakeBindingMutable,
                Some(local_decl.source_info.span),
                vec![(local_decl.source_info.span.shrink_to_lo(), "mut ".to_string())],
            );
        }
        err.help(
            "making the binding mutable is the minimal repair; if it must remain shared, \
             consider `RefCell` (or `Rc<RefCell<_>>` for shared ownership) in single-threaded \
             code, and `Mutex` or `RwLock` behind `Arc` across threads",
        );
    }

    fn record_interior_mutation_intent(&self, local: Local, trigger_span: Span) -> Option<Span> {
        let local_decl = &self.body.local_decls[local];
        let pat_span = match *local_decl.local_info() {
            LocalInfo::User(BindingForm::Var(mir::VarBindingForm {
                binding_mode: BindingMode(ByRef::No, Mutability::Not),
        ...
        let Some(body_owner) = self.body.source.def_id().as_local() else {
            return None;
        };
        let Some(body) = self.infcx.tcx.hir_maybe_body_owned_by(body_owner) else {
            return None;
        };
        let Some(let_hir_id) = (BindingFinder { span: pat_span }).visit_body(&body).break_value()
        else {
            return None;
        };
        let hir::Node::LetStmt(hir::LetStmt {
            pat:
                hir::Pat {
                    kind: hir::PatKind::Binding(_, binding_hir_id, ident, None),
                    span: binding_span,
                    ..
                },
            source: hir::LocalSource::Normal,
            ..
        }) = self.infcx.tcx.hir_node(let_hir_id)
        else {
            return None;
        };
        let source = if local_decl.ty.is_box() {
            BorrowckWrapperSource::Box
        } else if let ty::Adt(def, _) = local_decl.ty.kind()
            && self.infcx.tcx.is_diagnostic_item(sym::Rc, def.did())
        {
            BorrowckWrapperSource::Rc
        } else {
            BorrowckWrapperSource::Plain
        };
        self.infcx.tcx.sess.record_borrowck_wrapper_intent(BorrowckWrapperIntent {
            body_owner,
            binding_hir_id: *binding_hir_id,
            binding_span: *binding_span,
            trigger_span,
            binding_name: ident.name.to_string(),
            source,
            requirement: BorrowckWrapperRequirement::InteriorMutation,
        });
        Some(*binding_span)
    }

    fn record_interior_mutation_intent_by_span(&self, body_expr: &hir::Expr<'_>, error_span: Span) {
        let typeck = self.infcx.tcx.typeck(self.mir_def_id());
        let mut finder = MutationReceiverFinder { error_span, result: None };
        finder.visit_expr(body_expr);
        let Some(receiver) = finder.result else { return };
        let hir::ExprKind::Path(qpath) = receiver.kind else { return };
        let hir::def::Res::Local(binding_hir_id) = typeck.qpath_res(&qpath, receiver.hir_id) else {
            return;
        };
        let hir::Node::Pat(hir::Pat {
            kind: hir::PatKind::Binding(_, _, ident, None),
            span: binding_span,
            ..
        }) = self.infcx.tcx.hir_node(binding_hir_id)
        else {
            return;
        };
        let Some(local) =
            self.infcx.tcx.hir_parent_iter(binding_hir_id).find_map(|(_, node)| match node {
        ...
        let Some(init_ty) = typeck.expr_ty_opt(init) else { return };
        let source = if init_ty.is_box() {
            BorrowckWrapperSource::Box
        } else if let ty::Adt(def, _) = init_ty.kind()
            && self.infcx.tcx.is_diagnostic_item(sym::Rc, def.did())
        {
            BorrowckWrapperSource::Rc
        } else {
        ...
        self.infcx.tcx.sess.record_borrowck_wrapper_intent(BorrowckWrapperIntent {
            body_owner: self.mir_def_id(),
            binding_hir_id,
            binding_span: *binding_span,
            trigger_span: error_span,
            binding_name: ident.name.to_string(),
            source,
            requirement: BorrowckWrapperRequirement::InteriorMutation,
        });
...
struct MutationReceiverFinder<'hir> {
    error_span: Span,
    result: Option<&'hir hir::Expr<'hir>>,
}

impl<'hir> Visitor<'hir> for MutationReceiverFinder<'hir> {
    fn visit_expr(&mut self, expr: &'hir hir::Expr<'hir>) {
        if self.result.is_none()
            && expr.span.contains(self.error_span)
            && let hir::ExprKind::MethodCall(_, receiver, _, _) = expr.kind
            && matches!(receiver.kind, hir::ExprKind::Path(_))
        {
