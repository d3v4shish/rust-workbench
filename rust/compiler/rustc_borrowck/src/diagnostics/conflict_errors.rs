...
struct SimpleLetBindingFinder {
    pat_span: Span,
}

impl<'tcx> Visitor<'tcx> for SimpleLetBindingFinder {
    type Result = ControlFlow<(hir::HirId, Span, Symbol)>;

    fn visit_stmt(&mut self, stmt: &'tcx hir::Stmt<'tcx>) -> Self::Result {
        if let hir::StmtKind::Let(hir::LetStmt {
            pat: hir::Pat { kind: hir::PatKind::Binding(_, binding_hir_id, ident, None), span, .. },
            source: hir::LocalSource::Normal,
            ..
        }) = stmt.kind
            && *span == self.pat_span
        {
            ControlFlow::Break((*binding_hir_id, *span, ident.name))
        } else {
...
impl<'diag, 'tcx> MirBorrowckCtxt<'_, 'diag, 'tcx> {
    pub(crate) fn report_use_of_moved_or_uninitialized(
        ...
        if move_out_indices.is_empty() {
            ...
            if moved_place.projection.is_empty() {
                self.record_box_shared_ownership_intent(moved_place.local, span);
            }
    ...
    fn record_box_shared_ownership_intent(&self, local: mir::Local, trigger_span: Span) {
        let local_decl = &self.body.local_decls[local];
        if !local_decl.ty.is_box() {
            return;
        }
        let pat_span = match *local_decl.local_info() {
            LocalInfo::User(BindingForm::Var(VarBindingForm { pat_span, .. })) => pat_span,
            _ => return,
        };
        ...
        let Some((binding_hir_id, binding_span, name)) =
            (SimpleLetBindingFinder { pat_span }).visit_body(&body).break_value()
        else {
            return;
        };
        self.infcx.tcx.sess.record_borrowck_wrapper_intent(BorrowckWrapperIntent {
            ...
            source: BorrowckWrapperSource::Box,
            requirement: BorrowckWrapperRequirement::SharedOwnership,
        });
    }
    ...
    fn suggest_cloning_inner(
        ...
        if let hir::ExprKind::Path(hir::QPath::Resolved(
            None,
            Path { res: Res::Local(binding_hir_id), .. },
        )) = &inner_expr.kind
        {
            let binding_span = match tcx.hir_node(*binding_hir_id) {
                hir::Node::Pat(pat) => Some(pat.span),
                _ => None,
            };
            tcx.sess.record_borrowck_repair_for_binding(
                BorrowckRepairKind::CloneMovedValue,
                binding_span,
                sugg.clone(),
            );
            if ty.is_box()
                && let Some(binding_span) = binding_span
                && let hir::Node::Pat(hir::Pat {
                    kind: hir::PatKind::Binding(_, _, ident, None),
                    ..
                }) = tcx.hir_node(*binding_hir_id)
            {
                tcx.sess.record_borrowck_wrapper_intent(BorrowckWrapperIntent {
                    body_owner: self.mir_def_id(),
                    binding_hir_id: *binding_hir_id,
                    binding_span,
                    trigger_span: expr.span,
        ...
        if !matches!(
            ty.kind(),
            ty::Adt(def, _)
                if [tcx.get_diagnostic_item(sym::Arc), tcx.get_diagnostic_item(sym::Rc)]
                    .contains(&Some(def.did()))
        ) {
            err.help(
                "if shared ownership is intended, consider changing the surrounding APIs to use \
                 `Rc` in single-threaded code or `Arc` when the value crosses threads; combine \
                 these with `RefCell`, `Mutex`, or `RwLock` only when shared mutation is required",
            );
        }
        true
    }
