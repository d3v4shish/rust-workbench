...
impl<'tcx> RegionInferenceContext<'tcx> {
    ...
    /// Iterates the MIR locations contained in `r` without probing every location in the body.
    /// Tooling that needs a complete lifetime trace should prefer this to repeated
    /// [`Self::region_contains_point`] calls.
    pub(crate) fn region_locations(&self, r: RegionVid) -> impl Iterator<Item = Location> + '_ {
        let scc = self.constraint_sccs.scc(r);
        self.scc_values.locations_outlived_by(scc)
    }
