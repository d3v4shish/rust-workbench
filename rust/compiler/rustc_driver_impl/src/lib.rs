...
pub fn run_compiler(at_args: &[String], callbacks: &mut (dyn Callbacks + Send)) {
    ...
    if sopts.unstable_opts.borrowck_autofix_wrapper_variants
        && sopts.unstable_opts.borrowck_autofix.is_none()
    {
        default_early_dcx
            .early_fatal("-Zborrowck-autofix-wrapper-variants requires -Zborrowck-autofix=<PATH>");
    }
    if sopts.unstable_opts.borrowck_wrapper_suggestions
        && sopts.unstable_opts.borrowck_autofix.is_some()
    {
        default_early_dcx.early_fatal(
            "-Zborrowck-wrapper-suggestions cannot be combined with -Zborrowck-autofix",
        );
    }
    ...
    if has_input
        && !help_only
        && (sopts.unstable_opts.borrowck_ownership_events
            || sopts.unstable_opts.borrowck_wrapper_suggestions)
        && borrowck_autofix::should_orchestrate()
    {
        match borrowck_autofix::run_suggestions(&args) {
            Ok(borrowck_autofix::AutofixOutcome::Success) => return,
            Ok(borrowck_autofix::AutofixOutcome::CompilationFailed) => {
                drop(default_early_dcx);
                rustc_errors::FatalError.raise();
            }
            Err(error) => default_early_dcx.early_fatal(format!(
                "failed to validate borrow-checker wrapper suggestions: {error}"
            )),
        }
    }
    ...
    if has_input
        && !help_only
        && let Some(output_root) = sopts.unstable_opts.borrowck_autofix.as_deref()
        && borrowck_autofix::should_orchestrate()
    {
        match borrowck_autofix::run(&args, output_root) {
            Ok(borrowck_autofix::AutofixOutcome::Success) => return,
            Ok(borrowck_autofix::AutofixOutcome::CompilationFailed) => {
                drop(default_early_dcx);
                rustc_errors::FatalError.raise();
            }
            Err(error) => default_early_dcx
                .early_fatal(format!("failed to run borrow-checker autofix: {error}")),
        }
    }
    ...
    if let Err(error) = borrowck_autofix::install_overlay(&mut config) {
        default_early_dcx.early_fatal(format!("failed to load borrow-checker overlay: {error}"));
    }
