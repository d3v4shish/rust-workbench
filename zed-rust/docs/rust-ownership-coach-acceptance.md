...
- Schema 7 compiler artifacts record a move/copy destination when MIR proves it and separate
  variables, inline handles/wrappers, access gates, allocations, and buffers. Older artifacts remain
  readable through defaulted protocol fields.
- The analyzer exposes bounded schema 14 models with the compiler schema version, `valueTrace`, and
  a structured mutation requirement.
...
- Conceptual C remains collapsed under advanced evidence and is labeled as intent rather than
  compiler output, ABI-equivalent C, or guaranteed-to-compile C.
...
- `String`/`Vec`: variable → inline header → heap buffer; moving the header leaves the buffer in place.
- `Box<T>`: variable → unique inline handle → one heap allocation.
- `&T`/`&mut T`: variable → non-owning handle → live referent; shared versus exclusive access is explicit.
- `Rc<T>`/`Arc<T>`: variable → inline handle → shared allocation → inner value; count is symbolic.
- `RefCell<T>`: variable/owner → runtime borrow gate → inner value; conflicting access may panic.
...
- Editing is debounced; LSP, compiler, Cargo, and filesystem work stays off the UI
  thread.
- `script/benchmark-rust-learning-context` requires schema 14, a non-empty value trace for
