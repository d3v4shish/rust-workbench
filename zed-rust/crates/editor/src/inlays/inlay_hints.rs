...
use text::{Bias, BufferId, ToPoint as _};
...
pub enum InlayHintRefreshReason {
    ...
    RustOwnershipDisplayChanged,
}

impl Editor {
    pub fn rust_ownership_display_preferences(&self) -> &RustOwnershipDisplayPreferences {
        &self.rust_ownership_display
    }

    pub fn set_rust_ownership_display_preferences(
        &mut self,
        preferences: RustOwnershipDisplayPreferences,
        cx: &mut Context<Self>,
    ) {
        if self.rust_ownership_display == preferences {
            return;
        }
        let ownership_coloring_changed = self.rust_ownership_display.show_ownership_coloring
            != preferences.show_ownership_coloring;
        self.rust_ownership_display = preferences;
        self.refresh_inlay_hints(InlayHintRefreshReason::RustOwnershipDisplayChanged, cx);
        if ownership_coloring_changed {
    ...
    pub(crate) fn refresh_inlay_hints(
        ...
        let debounce = match &reason {
            ...
            | InlayHintRefreshReason::RustOwnershipDisplayChanged => None,
        ...
        let ignore_previous_fetches = match reason {
            ...
            | InlayHintRefreshReason::Toggle(_)
            | InlayHintRefreshReason::SettingsChange(_)
            | InlayHintRefreshReason::RustOwnershipDisplayChanged
    ...
    fn apply_fetched_hints(
        ...
        let visible_inlay_hint_ids = Self::visible_inlay_hints(self.display_map.read(cx))
            ...
            .map(|inlay| inlay.id)
            .collect::<Vec<_>>();
        let rust_ownership_display = self.rust_ownership_display.clone();
...
fn rust_workbench_hint_allowed(
    preferences: &RustOwnershipDisplayPreferences,
    hint: &InlayHint,
    ...
    let Some(kind) = hint.kind else {
        return preferences.show_other_hints;
    };
    match kind {
        InlayHintKind::Type => preferences.show_type_hints,
        InlayHintKind::Parameter => preferences.show_parameter_hints,
        InlayHintKind::Adjustment => {
            preferences.show_adjustments
                && preferences.allows_row(hint.position.to_point(buffer_snapshot).row)
        }
        InlayHintKind::Lifetime => {
            preferences.show_lifetimes
                && preferences.allows_row(hint.position.to_point(buffer_snapshot).row)
        }
        InlayHintKind::Drop => {
            preferences.show_drops
                && preferences.allows_row(hint.position.to_point(buffer_snapshot).row)
        }
        InlayHintKind::OwnershipEstimated | InlayHintKind::OwnershipExact => {
            if !preferences.allows_row(hint.position.to_point(buffer_snapshot).row) {
                return false;
            }
...
fn rust_ownership_event_allowed(
    preferences: &RustOwnershipDisplayPreferences,
    label: &str,
) -> bool {
    label.split('·').any(|event| {
        let event = event.trim();
        if event.contains("invalid use") {
            preferences.show_invalid_uses
        } else if event.contains("partial move") || event.contains("move") {
            preferences.show_moves
        } else if event.contains("borrow ends") {
            preferences.show_borrow_ends
        } else if event.contains("borrow") || event.contains("&?") || event.contains("&mut?") {
            preferences.show_borrows
        } else if event.contains("last") {
            preferences.show_last_uses
        } else if event.contains("reinit") {
...
pub mod tests {
    ...
    use crate::{Editor, RustOwnershipDisplayPreferences, SelectionEffects};
    ...
    #[test]
    fn rust_ownership_filters_keep_event_categories_independent() {
        let focus = RustOwnershipDisplayPreferences::focus();
        assert!(rust_ownership_event_allowed(&focus, "move?"));
        assert!(rust_ownership_event_allowed(&focus, "shared borrow"));
        assert!(rust_ownership_event_allowed(
            &focus,
            "invalid use after move"
        ));
        assert!(!rust_ownership_event_allowed(&focus, "borrow ends"));
        assert!(!rust_ownership_event_allowed(&focus, "last use"));
        assert!(!rust_ownership_event_allowed(&focus, "drop"));
        assert!(rust_ownership_event_allowed(
            &focus,
            "borrow ends · invalid use after move"
        ));
