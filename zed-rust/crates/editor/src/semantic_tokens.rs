            ...
            editor
                .update(cx, |editor, cx| {
                    ...
                    for (buffer_id, query_version, tokens) in all_semantic_tokens {
                        ...
                        let rust_display = editor.rust_ownership_display_preferences();
                        let show_ownership_coloring =
                            !rust_display.enabled || rust_display.show_ownership_coloring;
...
fn buffer_into_editor_highlights<'a, 'b>(
    ...
    multi_buffer_snapshot
        ...
        .filter_map(move |((multi_buffer_start, multi_buffer_end), token)| {
            ...
            let token_modifiers = if show_ownership_coloring {
                token.token_modifiers
            } else {
                stylizer.without_modifiers(
                    token.token_modifiers,
                    &[
                        "invalidUse",
                        "moved",
            ...
            let style = convert_token(stylizer, theme, token.token_type, token_modifiers)?;
