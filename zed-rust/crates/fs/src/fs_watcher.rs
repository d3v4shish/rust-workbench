...
fn register_existing_path(
    ...
    let Some((registration_id, mode)) = global_watcher().add_with_poll_fallback(
        path,
        mode,
        case_insensitive,
...
impl GlobalWatcher {
    /// Register using the selected backend, transparently switching to the recursive poll
    /// backend if Linux has exhausted its native inotify watch budget. This keeps settings,
    /// extension, and project paths live without a retry loop that wakes every few seconds.
    #[must_use]
    fn add_with_poll_fallback(
        &self,
        path: Arc<std::path::Path>,
        mode: WatcherMode,
        case_insensitive: bool,
        cb: impl Fn(&notify::Event) + Clone + Send + Sync + 'static,
    ) -> anyhow::Result<Option<(WatcherRegistrationId, WatcherMode)>> {
        if let Some(id) = self.add(path.clone(), mode, case_insensitive, cb.clone())? {
        ...
        log::warn!("native file watch capacity is exhausted for {path:?}; using the poll watcher");
        telemetry::event!(
            "fs_watcher_watch_limit_poll_fallback",
            path = path.display().to_string()
        );
        Ok(self
            .add(path, WatcherMode::Poll, case_insensitive, cb)?
            .map(|id| (id, WatcherMode::Poll)))
    }
    ...
    fn start_native_watch_limit_cooldown(&self, path: &Path) {
        ...
        if should_log {
            log::warn!(
                "OS file watch limit reached while watching {path:?}; new registrations will use polling for {} seconds",
...
mod tests {
    ...
    #[test]
    fn native_watch_limit_falls_back_to_polling() {
        let native_backend = Arc::new(Mutex::new(FakeWatchBackend {
            fail_with_watch_limit: true,
            ..Default::default()
        }));
        ...
        let path = Arc::<Path>::from(Path::new("/repo/project"));

        let (registration, mode) = watcher
            .add_with_poll_fallback(path.clone(), WatcherMode::Native, false, |_| {})
            .expect("watch fallback succeeds")
            .expect("poll registration is installed");

        assert_eq!(mode, WatcherMode::Poll);
        assert_eq!(native_backend.lock().watch_calls, &[path.to_path_buf()]);
        assert_eq!(poll_backend.lock().watch_calls, &[path.to_path_buf()]);
        watcher.remove(registration);
        assert_eq!(poll_backend.lock().unwatch_calls, &[path.to_path_buf()]);
    }
