    ...
    pub fn merge_diagnostic_entries<'a>(
        ...
        for mut update in diagnostic_updates {
            ...
            let Some((worktree, relative_path)) =
                ...
                // Language servers clear cached diagnostics for dependency and sysroot files as
                // they rebuild their world. Those files deliberately have no project worktree,
                // and an empty clear is neither actionable nor exceptional for the editor.
                if !update.diagnostics.diagnostics.is_empty() {
                    log::warn!(
                        "skipping non-empty diagnostics update, no worktree found for path {abs_path:?}"
                    );
                }
                continue;
            };
            ...
            .fold(
                ...
                |mut acc, (server_id, uri, diagnostics, version, new_registration_id)| {
                    ...
                    let Some((worktree, relative_path)) =
                        ...
                        if !diagnostics.is_empty() {
                            log::warn!("skipping non-empty workspace diagnostics update, no worktree found for path {abs_path:?}");
                        }
                        return acc;
                    };
                    let worktree_id = worktree.read(cx).id();
