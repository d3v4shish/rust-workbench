...
mod rust_workbench_inlay_tests {
    ...
    #[test]
    fn rust_workbench_metadata_separates_exact_and_estimated_ownership() {
        let estimated = serde_json::json!({
            "rustWorkbench": {
                "version": 1,
                "category": "ownership",
                "precision": "analyzer_estimate"
            }
        });
        ...
        assert_eq!(
            rust_workbench_inlay_kind(Some(&estimated)),
            Some(InlayHintKind::OwnershipEstimated)
        );
        assert_eq!(
            rust_workbench_inlay_kind(Some(&exact)),
...
fn rust_workbench_inlay_kind(data: Option<&lsp::LSPAny>) -> Option<InlayHintKind> {
    ...
    match metadata.get("category")?.as_str()? {
        "adjustment" => Some(InlayHintKind::Adjustment),
        "lifetime" => Some(InlayHintKind::Lifetime),
        "ownership" => match metadata.get("precision").and_then(|value| value.as_str()) {
            Some("compiler_exact") => Some(InlayHintKind::OwnershipExact),
...
impl InlayHints {
    pub async fn lsp_to_project_hint(
        ...
        let kind = lsp_hint
            ...
            .or_else(|| rust_workbench_inlay_kind(lsp_hint.data.as_ref()));
    ...
    pub fn project_to_lsp_hint(hint: InlayHint, snapshot: &BufferSnapshot) -> lsp::InlayHint {
        lsp::InlayHint {
            ...
            kind: hint.kind.and_then(|kind| match kind {
                InlayHintKind::Type => Some(lsp::InlayHintKind::TYPE),
                InlayHintKind::Parameter => Some(lsp::InlayHintKind::PARAMETER),
