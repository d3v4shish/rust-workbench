...
use rope::Rope;
use rust_workbench::{OwnershipCoachBanner, RustWorkbenchPanel};
...
fn initialize_panels(window: &mut Window, cx: &mut Context<Workspace>) -> Task<anyhow::Result<()>> {
    cx.spawn_in(window, async move |workspace_handle, cx| {
        ...
        let rust_workbench = RustWorkbenchPanel::load(workspace_handle.clone(), cx.clone());
        ...
        futures::join!(
            ...
            add_panel_when_ready(rust_workbench, workspace_handle.clone(), cx.clone()),
...
fn initialize_pane(
    ...
    pane.update(cx, |pane, cx| {
        pane.toolbar().update(cx, |toolbar, cx| {
            ...
            let ownership_coach_banner =
                cx.new(|_| OwnershipCoachBanner::new(workspace_handle.clone()));
            toolbar.add_item(ownership_coach_banner, window, cx);
...
fn open_about_window(cx: &mut App) {
    fn about_window_icon(_release_channel: ReleaseChannel) -> Arc<Image> {
        let bytes = include_bytes!("../resources/app-icon-rust-workbench.png").as_slice();
